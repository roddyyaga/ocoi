...
module Phone : Ppx_mysql_runtime.SERIALIZABLE with type t = string = struct
  ...
  let of_mysql str =
    if String.length str <= 9 then Ok str else Error "string too long"
...
type user = { id: int32; name: string; phone: Phone.t option }

let user_of_tuple (id, name, phone) = { id; name; phone }

let print_user { id; name; phone } =
  Lwt_io.printf "\t%ld -> %s (phone: %s)\n" id name
...
let get_all_users dbh =
  ...
  [%mysql
    select_all "SELECT @int32{id}, @string{name}, @string?{phone} FROM users"]
    dbh
...
let insert_users =
  [%mysql
    execute
      "INSERT INTO users (id, name, phone) VALUES %list{(%int32{id}, \
