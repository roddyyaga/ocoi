             ...
             (match Ppx_yojson_conv_lib.(!) extra with
              ...
              | [] ->
                  (match ((Ppx_yojson_conv_lib.(!) id_field),
                   ...
                   with
                   | (Some id_value, Some name_value, Some cool_value, Some
                      another_id_value) ->
                       {
                         id = id_value;
                         name = name_value;
                   ...
                   | _ ->
                       Ppx_yojson_conv_lib.Yojson_conv_error.record_undefined_elements
                         ...
                         ((Ppx_yojson_conv_lib.poly_equal
                           ...
                           "cool");
                         ((Ppx_yojson_conv_lib.poly_equal
                             (Ppx_yojson_conv_lib.(!) another_id_field) None),
                           "another_id")]))))
   | _ as yojson ->
...
let yojson_of_t =
  (function
   | { id = v_id; name = v_name; cool = v_cool; another_id = v_another_id }
       ...
       let bnds =
         let arg = yojson_of_int v_another_id in ("another_id", arg) :: bnds in
       let bnds = let arg = yojson_of_bool v_cool in ("cool", arg) :: bnds in
...
let t_no_id_of_yojson =
  (let _tp_loc = "test.ml.t_no_id" in
   ...
   | _ as yojson ->
       Ppx_yojson_conv_lib.Yojson_conv_error.record_list_instead_atom _tp_loc
         yojson : Ppx_yojson_conv_lib.Yojson.Safe.t -> t_no_id)
let _ = t_no_id_of_yojson
let t_no_id_of_yojson' =
  (let _tp_loc = "test.ml.t_no_id" in
   fun yojson ->
     Ppx_yojson_conv_lib.Yojson_conv.Result.pack
       (fun yojson -> t_no_id_of_yojson yojson) yojson : Ppx_yojson_conv_lib.Yojson.Safe.t
                                                           ->
                                                           t_no_id
                                                             Ppx_yojson_conv_lib.Yojson_conv.Result.t)
let _ = t_no_id_of_yojson'
